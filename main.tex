%% Elsevier cas-dc version of your paper

\documentclass[review,5p]{cas-dc}
...
\usepackage{pgfplots}
\usepackage{tabularx}  
\usepackage{caption}  % or capt-of if caption clashes with the class
...
\author[1]{Dat Thanh Tran\corref{cor1}}
\ead{dat.tt3@vinuni.edu.vn}
\author[1]{Khai Quang Tran}
...
\paragraph{Why tree-based representations?}
...
\begin{itemize}
    ...
    introduces covering sequences~\citep{yehezkeally2025covering} and covering 2D-sequences, where all
...
\paragraph{Texture Synthesis and Image Stitching.}
...
\paragraph{Jigsaw Puzzle Assembly and Polyomino Packing.}
...
Whenever the distinction between the two variants is not important, we simply refer to either as \emph{2D-SSP}.

\paragraph{Terminology.} Hereafter, we use \emph{string} to mean \emph{2D string} unless otherwise specified.
...
\[
  x_1^{\max} = \max\{x : (x,y) \in B_1 \text{ for some } y\}, \quad
...
\paragraph{Choice of solution representation.}
...
\paragraph{Global grid, symmetry breaking, and greedy-based bounds.}
...
\paragraph{Variant 1: square objective.}
...
\paragraph{Variant 2: area objective and linearization.}
...
\paragraph{Decoding a placement tree.}
...
\paragraph{Initialization from greedy placements.}
...
\paragraph{Tree-based crossover with greedy completion.}
...
\paragraph{Main evolutionary loop.}
...
\paragraph{Design choice: no explicit mutation operator.}
...
improving on deterministic greedy by about $6\%$ on average, at the cost of
one to two orders of magnitude more runtime (tens of seconds vs.\ a few
tens of milliseconds).  The greedy heuristics remain essentially linear-time
and are preferred when runtime is the dominant concern.

\paragraph{Scaling curves.}
...
%% Bibliography
%% You can change style and bib file name as needed
